...
As seen in \cref{fig:inst} there are 4 bits allocated to opcodes. \cref{tbl:ISA} accounts for fourteen instructions, all assigned an OPCODE shown in \cref{tbl:opcode}.
\begin{table}[H]
    \centering
...
\cref{lst:ass} shows the erosion algorithm compiled to assembler using the ISA provided in \cref{tbl:ISA}.
\begin{figure}
...
The instruction set in \cref{tbl:ISA} with opcodes in \cref{tbl:opcode} is encoded using the instruction scheme in \cref{fig:inst} as illustrated in \cref{tbl:encex}. The program in \cref{lst:ass} is encoded to machine code in \cref{lst:machine}.
\begin{table}[H]
    \centering
