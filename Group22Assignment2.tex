...
The instructionset in \cref{tbl:ISA} with opcodes in \cref{tbl:opcode} is encoded using the instruction scheme in \cref{fig:inst} as illustrated in \cref{tbl:encex}. The program in \cref{lst:ass} is encoded to machine code in \cref{lst:machine}.
\begin{table}[H]
    \centering
    \caption{Encoding instructions under the instruction set and encoding scheme}\label{tbl:encex}
    \begin{tabular}{llllll}
        \toprule
        \textbf{Instruction}     & OPCODE        & Rd            & Ra            & Rb            & Value/adress                 \\
        \midrule
