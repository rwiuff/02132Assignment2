...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        Jump if equal            & \texttt{JGT Rx, Ry, z;}  & \texttt{if(Rx > Ry) GOTO INST z} \\
...
\begin{figure}[!h]
    ...
    \begin{minipage}{0.49\textwidth}
        \vspace{1.5em}
        \centering
        \begin{minted}[firstnumber = 0]{mips}
        # Initial values
LOADI R0, 0;      # x counter
LOADI R1, 0;      # y counter
LOADI R2, 19;     # Pixel limit
LOADI R3, 0;      # Zero value
LOADI R4, 255;    # 255 value
        # For loop conditions
JEQ R0, R2, 58    # Check x, GOTO END
JEQ R1, R2, 54    # Check y, GOTO INC X
        # Output image adress
MULT R5, R1, 20;  # y * 20
ADD R6, R0, R5;   # x + y * 20
ADDI R5, R6, 400; # Out image address
        # Process border pixel
JEQ R0, R3, 48    # If x or y = 0
JEQ R1, R3, 48    #
JEQ R0, R4, 48    # If x or y = 19
JEQ R1, R4, 48    # GOTO erosion
        # Process inner pixel
MULT R6, R1, 20;  # y * 20
ADD R7, R0, R6;   # x + y * 20
LOAD R8, R7;      # Get input pixel
JEQ R8, R3, 48;   # If 0, GOTO erosion
...
ADDI R15, R1, 1   # y + 1
MULT R6, R1, 20;  # y * 20
ADD R7, R6, R12;  # (x - 1) + y * 20
    \end{minted}
    \end{minipage}
    \begin{minipage}{0.49\textwidth}
        \vspace{.5em}
        \centering
        \begin{minted}[firstnumber = 30]{mips}
LOAD R8, R7;      # Save pixel in R8
MULT R6, R1, 20;  # y * 20
ADD R7, R6, R13;  # (x + 1) + y * 20
LOAD R9, R7;      # Save pixel in R9
OR R10, R8, R9;   # OR R8 and R9, save to R10
MULT R6, R14, 20; # (y - 1) * 20
ADD R7, R6, R0;   # x + (y - 1) * 20
LOAD R8, R7;      # Save pixel in R8
OR R9, R8, R10;   # OR R8 nad R10, save to R9
MULT R6, R15, 20; # (y + 1) * 20
ADD R7, R6, R0;   # x + (y + 1) * 20
LOAD R10, R7;     # Save pixel in R10
OR R8, R9, R10;   # OR R9 and R10, save to R8
JEQ R8, R3, 48;   # If = 0 GOTO Erosion
            # No erosion
STORE R4, R5;     # Set pixel to 255
JMP 51;           # GOTO increment y
            # Erosion
STORE R3, R5;     # Set Pixel to zero
JMP 51;           # GOTO increment y
            # Increment y
INC R1;           # Increment y
JMP 8;            # Continue nested loop
            # Increment x
INC R0;           # Increment x
LOADI R1, 0;      # Zerorise y
JMP 7;            # Continue main loop
            # Terminate program
